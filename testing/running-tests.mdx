...
<Accordion title="CSS imports are handled by identity-obj-proxy">
  Vite handles CSS Modules at build time, but Jest doesn't run through Vite. Without a mock, `import styles from './Hero.module.css'` throws a syntax error. `identity-obj-proxy` is mapped via `moduleNameMapper` in `jest.config.cjs` to intercept those imports and return an object whose keys match whatever class name is accessed — so `styles.card` returns the string `"card"` and no error is thrown.

  You do **not** need to add anything to individual test files for this to work; the proxy operates globally for all tests.
</Accordion>
