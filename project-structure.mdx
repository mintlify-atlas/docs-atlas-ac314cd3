...
**`src/main.jsx`** is the Webpack entry point rendered into `index.html`. It wraps `HeroesApp` in `React.StrictMode` and a `HashRouter`, which means every route in the app is prefixed with `#/` — enabling deployment to any static host without server-side rewrites.
...
Examples: `marvel-spider.jpg`, `dc-batman.jpg`, `marvel-iron.jpg`, `dc-wonderwoman.jpg`.
